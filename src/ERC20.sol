...
contract TokenTransfer {
    ...
    // Note: You're forcing the caller to provide the token address, sender, recipient, and amount when creating a contract. This is not necessary.
    // Note: These variables don't need to public, these are args to transfer_check()  which are passed down to _safeTransfer()
    // Note: So you can keep an empty constructor() and remove these variables
    constructor(address _token, address _sender, address _recipient, uint256 _amount) {
        token = IERC20(_token);
        sender = _sender;
    ...
    // Note: Use camelcase for function names in solidity, i.e. transferCheck() or TransferCheck()
    // Note: Use underscore for private functions, i.e. _safeTransfer()
    // Note: Use underscore for private variables, i.e. _token, _sender, _recipient, _amount
    // Note: transfer..check() implies that this function checks something, but it doesn't. It actually performs the transfer. So it should have a more appropriate name like transfer() or transferTokens()
    ...
    // Note: you have a conflict between argument variables and state variables.
